...
######################
    ...
    def setup_headerbar_content(self, win):
        ...
        popover = menu_button.get_popover()
        if popover:
            popover.connect("closed", lambda *args: win.webview.grab_focus())
    ...
    def on_show_shortcuts(self, action, param):
        """Show keyboard shortcuts dialog using GTK's shortcuts window"""
        if not self.windows:
            return
        ...
        shortcuts_window.set_transient_for(active_win)
        ...
        section.set_property("visible", True)  # Ensure section is visible
        
        # File Operations group
        file_group = Gtk.ShortcutsGroup()
        file_group.set_property("title", "File Operations")
        file_group.set_property("visible", True)
        
        file_shortcuts = [
            ("New Document", "<Primary>n"),
            ("Open Document", "<Primary>o"),
            ("Save Document", "<Primary>s"),
            ("Save As", "<Shift><Primary>s"),
            ("Print Document", "<Primary>p"),
            ("Close Document", "<Primary>w"),
            ("Close Others", "<Shift><Primary>w"),
        ]
        
        for title, accelerator in file_shortcuts:
        
            shortcut = Gtk.ShortcutsShortcut()
            shortcut.set_property("title", title)
            shortcut.set_property("accelerator", accelerator)
            shortcut.set_property("visible", True)
            file_group.add_shortcut(shortcut)
        
        section.add_group(file_group)
        
        # Text Formatting group
        formatting_group = Gtk.ShortcutsGroup()
        formatting_group.set_property("title", "Text Formatting")
        formatting_group.set_property("visible", True)
        
        formatting_shortcuts = [
            ("Bold", "<Primary>b"),
            ("Italic", "<Primary>i"),
            ("Underline", "<Primary>u"),
            ("Strikeout", "<Shift><Primary>x"),
            ("Superscript", "<Shift><Primary>equal"),
            ("Subscript", "<Shift><Primary>minus")
        ]
        
        for title, accelerator in formatting_shortcuts:
            print("Adding shortcut:", title, accelerator)
            shortcut = Gtk.ShortcutsShortcut()
            shortcut.set_property("title", title)
            shortcut.set_property("accelerator", accelerator)
            shortcut.set_property("visible", True)
            formatting_group.add_shortcut(shortcut)
        
        section.add_group(formatting_group)
        
        # Alignment group
        alignment_group = Gtk.ShortcutsGroup()
        alignment_group.set_property("title", "Text Alignment")
        alignment_group.set_property("visible", True)
        ...
        for title, accelerator in alignment_shortcuts:
            shortcut = Gtk.ShortcutsShortcut()
            shortcut.set_property("title", title)
            shortcut.set_property("accelerator", accelerator)
            shortcut.set_property("visible", True)
            alignment_group.add_shortcut(shortcut)
        
        section.add_group(alignment_group)
        
        # Lists group
        lists_group = Gtk.ShortcutsGroup()
        lists_group.set_property("title", "Lists")
        lists_group.set_property("visible", True)
        
        lists_shortcuts = [
            ("Numbered List", "F12"),
            ("Numbered List Alt", "<Shift><Primary>7"),
            ("Bullet List", "<Shift>F12"),
            ("Bullet List Alt", "<Shift><Primary>8")
        ]
        
        for title, accelerator in lists_shortcuts:
            shortcut = Gtk.ShortcutsShortcut()
            shortcut.set_property("title", title)
            shortcut.set_property("accelerator", accelerator)
            shortcut.set_property("visible", True)
            lists_group.add_shortcut(shortcut)
        
        section.add_group(lists_group)
        
        # Find and Replace group
        find_group = Gtk.ShortcutsGroup()
        find_group.set_property("title", "Find and Replace")
        find_group.set_property("visible", True)
        ...
        for title, accelerator in find_shortcuts:
            shortcut = Gtk.ShortcutsShortcut()
            shortcut.set_property("title", title)
            shortcut.set_property("accelerator", accelerator)
            shortcut.set_property("visible", True)
            find_group.add_shortcut(shortcut)
        
        section.add_group(find_group)
        
        # Copy, Cut, Paste group
        clipboard_group = Gtk.ShortcutsGroup()
        clipboard_group.set_property("title", "Clipboard")
        clipboard_group.set_property("visible", True)
        ...
        for title, accelerator in clipboard_shortcuts:
            shortcut = Gtk.ShortcutsShortcut()
            shortcut.set_property("title", title)
            shortcut.set_property("accelerator", accelerator)
            shortcut.set_property("visible", True)
            clipboard_group.add_shortcut(shortcut)
        
        section.add_group(clipboard_group)
        
        # Undo and Redo group
        undo_redo_group = Gtk.ShortcutsGroup()
        undo_redo_group.set_property("title", "Undo and Redo")
        undo_redo_group.set_property("visible", True)
        
        undo_redo_shortcuts = [
            ("Undo", "<Primary>z"),
            ("Redo", "<Shift><Primary>z"),
            ("Redo Alt", "<Primary>y")
        ]
        
        for title, accelerator in undo_redo_shortcuts:
            shortcut = Gtk.ShortcutsShortcut()
            shortcut.set_property("title", title)
            shortcut.set_property("accelerator", accelerator)
            shortcut.set_property("visible", True)
            undo_redo_group.add_shortcut(shortcut)
        
        section.add_group(undo_redo_group)
        
        # Zoom group
        zoom_group = Gtk.ShortcutsGroup()
        zoom_group.set_property("title", "Zoom")
        zoom_group.set_property("visible", True)
        ...
        for title, accelerator in zoom_shortcuts:
            shortcut = Gtk.ShortcutsShortcut()
            shortcut.set_property("title", title)
            shortcut.set_property("accelerator", accelerator)
            shortcut.set_property("visible", True)
            zoom_group.add_shortcut(shortcut)
        
        section.add_group(zoom_group)
        
        # Font Size group
        font_size_group = Gtk.ShortcutsGroup()
        font_size_group.set_property("title", "Font Size")
        font_size_group.set_property("visible", True)
        
        font_size_shortcuts = [
            ("Increase Font Size", "<Primary>bracketright"),
            ("Decrease Font Size", "<Primary>bracketleft"),
            ("Increase Font Size (Large)", "<Shift><Primary>period"),
            ("Decrease Font Size (Large)", "<Shift><Primary>comma")
        ]
        
        for title, accelerator in font_size_shortcuts:
            shortcut = Gtk.ShortcutsShortcut()
            shortcut.set_property("title", title)
            shortcut.set_property("accelerator", accelerator)
            shortcut.set_property("visible", True)
            font_size_group.add_shortcut(shortcut)
        
        section.add_group(font_size_group)
        
        # Line Spacing group
        line_spacing_group = Gtk.ShortcutsGroup()
        line_spacing_group.set_property("title", "Line Spacing")
        line_spacing_group.set_property("visible", True)
        
        line_spacing_shortcuts = [
            ("Single Spacing", "<Primary>1"),
            ("1.5 Spacing", "<Primary>5"),
            ("Double Spacing", "<Primary>2"),
            ("Default Spacing", "<Shift><Primary>0")
        ]
        
        for title, accelerator in line_spacing_shortcuts:
            shortcut = Gtk.ShortcutsShortcut()
            shortcut.set_property("title", title)
            shortcut.set_property("accelerator", accelerator)
            shortcut.set_property("visible", True)
            line_spacing_group.add_shortcut(shortcut)
        
        section.add_group(line_spacing_group)
        
        # Paragraph Styles group
        styles_group = Gtk.ShortcutsGroup()
        styles_group.set_property("title", "Paragraph Styles")
        styles_group.set_property("visible", True)
        
        styles_shortcuts = [
            ("Normal", "<Alt><Primary>0"),
            ("Heading 1", "<Alt><Primary>1"),
            ("Heading 2", "<Alt><Primary>2"),
            ("Heading 3", "<Alt><Primary>3"),
            ("Heading 4", "<Alt><Primary>4"),
            ("Heading 5", "<Alt><Primary>5"),
            ("Heading 6", "<Alt><Primary>6")
        ]
        
        for title, accelerator in styles_shortcuts:
            shortcut = Gtk.ShortcutsShortcut()
            shortcut.set_property("title", title)
            shortcut.set_property("accelerator", accelerator)
            shortcut.set_property("visible", True)
            styles_group.add_shortcut(shortcut)
        
        section.add_group(styles_group)
        
        # Interface Toggles group
        interface_group = Gtk.ShortcutsGroup()
        interface_group.set_property("title", "Interface")
        interface_group.set_property("visible", True)
        
        interface_shortcuts = [
            ("Toggle File Toolbar", "<Alt>f"),
            ("Toggle Header Bar", "<Alt>h"),
            ("Toggle Status Bar", "<Alt>s"),
            ("Toggle Format Toolbar", "<Alt>t"),
            ("Page Options", "<Alt>p")
        ]
        
        for title, accelerator in interface_shortcuts:
            shortcut = Gtk.ShortcutsShortcut()
            shortcut.set_property("title", title)
            shortcut.set_property("accelerator", accelerator)
            shortcut.set_property("visible", True)
            interface_group.add_shortcut(shortcut)
        
        section.add_group(interface_group)
        
        # Add section to shortcuts window
...
############
    def on_show_shortcuts(self, action, param):
        ...
        dialog.get_content_area().append(main_box)  # Use append instead of pack_start
        
        # Create scrolled window
        scrolled = Gtk.ScrolledWindow()
        scrolled.set_policy(Gtk.PolicyType.NEVER, Gtk.PolicyType.AUTOMATIC)
        main_box.append(scrolled)  # Use append instead of pack_start
        
        # Create list box for groups
        self.listbox = Gtk.ListBox()
        self.listbox.set_selection_mode(Gtk.SelectionMode.NONE)
        scrolled.set_child(self.listbox)  # Use set_child instead of add
        
        # Define all shortcuts groups
        groups = [
            ("File Operations", [
                ("New Document", "<Primary>n"),
                ("Open Document", "<Primary>o"),
        ...
        # Store reference to all rows for filtering
        self.all_rows = []
        
        # Add groups to listbox
        for group_title, shortcuts in groups:
            group_row = self.create_group_row(group_title)
            self.listbox.append(group_row)  # Use append instead of add
            self.all_rows.append(group_row)
            
            for title, accelerator in shortcuts:
                shortcut_row = self.create_shortcut_row(title, accelerator)
                self.listbox.append(shortcut_row)  # Use append instead of add
                self.all_rows.append(shortcut_row)
    ...
    def create_group_row(self, title):
        row = Gtk.ListBoxRow()
        ...
        label.get_style_context().add_class("group-header")
        
        box.append(label)  # Use append instead of pack_start
        row.set_child(box)  # Use set_child instead of add
        
        # Store original title for search
        row.search_text = title.lower()
        row.is_group = True
        
        return row

    def create_shortcut_row(self, title, accelerator):
        row = Gtk.ListBoxRow()
        row.set_selectable(False)
        ...
        accel_label = Gtk.Label(label=accelerator)
        accel_label.set_halign(Gtk.Align.END)
        accel_label.get_style_context().add_class("keycap")
        
        box.append(title_label)  # Use append instead of pack_start
        box.append(accel_label)  # Use append instead of pack_end
        row.set_child(box)  # Use set_child instead of add
        
        # Store original text for search
        row.search_text = f"{title.lower()} {accelerator.lower()}"
        row.is_group = False
        row.parent_group = None  # Will be set during filtering
    ...
    def on_search_changed(self, entry):
        ...
        # First pass: show/hide shortcut rows and track visible groups
        visible_groups = set()
        for row in self.all_rows:
            if not row.is_group:
                if search_text in row.search_text:
                    row.set_visible(True)  # Use set_visible instead of show
                    # Find parent group
                    prev = row.get_prev_sibling()
                    while prev and not prev.is_group:
                        prev = prev.get_prev_sibling()
                    if prev:
                        visible_groups.add(prev)
                        row.parent_group = prev
                else:
                    row.set_visible(False)  # Use set_visible instead of hide
                    row.parent_group = None
        
        # Second pass: show group headers only if they have visible children
        for row in self.all_rows:
            if row.is_group:
                row.set_visible(row in visible_groups)  # Use set_visible instead of show/hide
