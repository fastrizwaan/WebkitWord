...
"""
    ...
    def on_activate(self, app):
        ...
        # Log LANG environment variable
        lang_env = os.getenv("LANG", "Not set")
        logger.info(f"LANG environment variable: {lang_env}")

        # Check if enchant-2 is installed and functional
        try:
            enchant_check = subprocess.run(["enchant-2", "-v"], capture_output=True, text=True)
            logger.info(f"Enchant status: {enchant_check.stdout.strip()}")
        except FileNotFoundError:
            logger.error("enchant-2 not found. Spell checking will not work. Install enchant-2 and a dictionary (e.g., hunspell-en-us).")

        # Create main window
        win = self.create_window()
        win.present()

        # Create WebView with spell check enabled
        context = WebKit.WebContext.get_default()
        context.set_spell_checking_enabled(False)

        # Set spell checking languages dynamically based on system locale
        languages = [system_locale.replace("_", "-")]  # Convert en_US to en-US
        context.set_spell_checking_languages(languages)
        logger.info(f"Setting spell check languages: {languages}")

        # Verify Enchant supports the specified languages
        for lang in languages:
            try:
                lang_for_enchant = lang.replace("-", "_")
                result = subprocess.run(["enchant-2", "-l", "-d", lang_for_enchant], input="teh", capture_output=True, text=True)
                if result.returncode != 0 or "teh" not in result.stdout:
                    logger.warning(f"Enchant dictionary for {lang} not found. Spell check may not work for this language.")
            except FileNotFoundError:
                logger.error(f"Enchant not found. Spell checking will not work.")
...
############# Create Window
    def create_window(self):
        ...
        win.file_toolbar = self.create_file_toolbar(win, win.webview)
...
##############
    ...
    def create_file_toolbar(self, win, webview):
        ...
        # --- Spell Check Group ---
        spell_check_group = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=0)
        spell_check_group.add_css_class("linked")
        spell_check_group.add_css_class("toolbar-group")
        
        # Spell Check Toggle button
        spell_check_button = Gtk.ToggleButton(icon_name="tools-check-spelling-symbolic")
        spell_check_button.set_tooltip_text("Toggle Spell Check")
        spell_check_button.set_active(False)  # Default: spell check disabled
        spell_check_button.connect("toggled", self.on_spell_check_toggled, webview)
        spell_check_button.set_size_request(40, 36)
        
        # Add spell check button to the group
        spell_check_group.append(spell_check_button)
        
        # Add the spell check group to the toolbar
        file_toolbar.append(spell_check_group)
        
        return file_toolbar

    def on_spell_check_toggled(self, button, webview):
        """Handle spell check toggle button state changes"""
        context = WebKit.WebContext.get_default()
        spell_check_enabled = button.get_active()
        context.set_spell_checking_enabled(spell_check_enabled)
        logger.info(f"Spell checking {'enabled' if spell_check_enabled else 'disabled'}")
        # Force WebView to update spell check state by re-running spell check
    ...
    def debug_spell_check(self, button, webview):
        logger.info("Running spell check diagnostics...")
        webview.evaluate_javascript("""
            function checkSpellStatus() {
                let editor = document.getElementById('editor');
                let misspelled = editor.innerText.match(/\\bteh\\b/) ? true : false;
                return {
                    documentLang: document.documentElement.lang,
                    editorSpellcheck: editor.spellcheck,
                    editorLang: editor.lang || 'Not set',
                    hasMisspelledWord: misspelled
                };
            }
            checkSpellStatus();
        """, -1, None, None, None, GLib.Callback(self.on_js_finished))

    def on_js_finished(self, webview, result):
        try:
            js_result = webview.evaluate_javascript_finish(result)
            if js_result:
                ...
                if value:
                    ...
                    logger.info(f"Spell check status: {result_str}")
        except Exception as e:
...
# Replace the _on_get_stack_sizes method with this more robust version:
    ...
    def show_general_page_setup_dialog(self, win):
        ...
        dialog.present(win)
######################
    # Include the modified get_editor_html and related methods here
    def get_editor_html(self, content=""):
        """Return HTML for the editor with improved table, text box styles, and spell check support"""
        content = self._prepare_content(content)
    ...
    def _get_editor_head(self, content):
        """Return the head section of the editor HTML with spell check support"""
        return f"""
            <title>Webkit Word</title>
            <meta charset="UTF-8">
            <style>
    ...
    def _get_editor_body(self):
        ...
        return """
            <div id="editor" contenteditable="true" spellcheck="true">
                Type here to test spell check. Here is a misspelled word: teh
            </div>
            <script>
                document.addEventListener('DOMContentLoaded', function() {
                    console.log("Document language: " + document.documentElement.lang);
                    console.log("Editor spellcheck attribute: " + document.getElementById('editor').spellcheck);
                });
            </script>
        """

    def _get_spell_check_styles(self):
        """Return CSS styles for spell checking underlines"""
        return """
            [spellcheck="true"] {
                text-decoration: none !important;
            }
            ::spelling-error {
                text-decoration: wavy red !important;
            }
        """
