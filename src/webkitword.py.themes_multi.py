...
######################
    ...
    def setup_css_provider(self, theme_key=None):
        """Set up CSS provider for custom styling with theme support"""
        if theme_key is None:
            theme_key = self.current_theme
        
        self.css_provider = Gtk.CssProvider()
        
        # Get CSS data for the specified theme
        css_data = self.get_theme_css(theme_key).encode('utf-8')
    ...
    def change_theme(self, theme_key):
        ...
        if self.set_current_theme(theme_key):
            self.setup_css_provider(theme_key)
            # Update status in all windows
            for window in self.windows:
                if hasattr(window, 'statusbar'):
                    theme_name = self.themes[theme_key]["name"]
                    window.statusbar.set_text(f"Theme changed to: {theme_name}")
            return True
        return False

    def on_preferences(self, action, param):
        """Show preferences dialog with theme selection and enhanced UI visibility options"""
        if not self.windows:
            return
        ...
        theme_header = Gtk.Label()
        theme_header.set_markup("<b>Theme</b>")
        theme_header.set_halign(Gtk.Align.START)
        theme_header.set_margin_bottom(12)
        content_box.append(theme_header)
        ...
        theme_keys = self.get_theme_keys()
        ...
        theme_dropdown.set_model(string_list)
        ...
        ok_button.add_css_class("suggested-action")
        ok_button.connect("clicked", lambda btn: self.save_preferences(
            dialog, active_win, auto_save_switch.get_active(), 
            spinner.get_value_as_int(), theme_dropdown, theme_keys
        ))
        button_box.append(ok_button)
    ...
    def save_preferences(self, dialog, win, auto_save_enabled, auto_save_interval, theme_dropdown, theme_keys):
        """Save preferences settings including theme"""
        previous_auto_save = win.auto_save_enabled
        ...
        selected_theme_index = theme_dropdown.get_selected()
        if selected_theme_index < len(theme_keys):
            selected_theme_key = theme_keys[selected_theme_index]
            if selected_theme_key != self.current_theme:
                self.change_theme(selected_theme_key)
    ...
    def create_actions(self):
        """Create application actions including theme switching"""
        # Existing actions...
        
        # Theme switching actions
        for theme_key in self.get_theme_keys():
            action_name = f"theme-{theme_key}"
            action = Gio.SimpleAction.new(action_name, None)
            action.connect("activate", lambda a, p, key=theme_key: self.change_theme(key))
            self.add_action(action)

    def setup_headerbar_content(self, win):
        """Create improved headerbar content with theme menu"""
        win.headerbar.set_margin_top(0)
        win.headerbar.set_margin_bottom(0)
        ...
        win.find_button_handler_id = win.find_button.connect("toggled", lambda btn: self.on_find_button_toggled(win, btn))
        win.find_button.set_size_request(40, 36)
        win.headerbar.pack_start(win.find_button)
        ...
        # Theme quick switch button (optional)
        theme_button = Gtk.MenuButton()
        theme_button.set_icon_name("applications-graphics-symbolic")
        theme_button.set_tooltip_text("Quick Theme Switch")
        theme_button.add_css_class("flat")
        
        # Create theme menu
        theme_menu = Gio.Menu()
        for i, theme_key in enumerate(self.get_theme_keys()):
            theme_name = self.themes[theme_key]["name"]
            theme_menu.append(theme_name, f"app.theme-{theme_key}")
        
        theme_button.set_menu_model(theme_menu)
        win.headerbar.pack_end(theme_button)
        
        # Create main menu button (last item on right side)
        menu_button = Gtk.MenuButton()
        menu_button.set_icon_name("open-menu-symbolic")
        ...
        popover = menu_button.get_popover()
        if popover:
            popover.connect("closed", lambda: win.webview.grab_focus())
...
#    def __init__(self):
    ...
    def get_default_theme(self):
        """Original theme with gray colors"""
        return {
            "name": "Default Gray",
            "css": """
.toolbar-container { padding: 0px 0px; background-color: rgba(127, 127, 127, 0.05); }
...
""" + self.get_common_css()
    ...
    def get_blue_theme(self):
        ...
        return {
            "name": "Ocean Blue",
            "css": """
.toolbar-container { padding: 0px 0px; background-color: rgba(52, 152, 219, 0.08); }
.flat { background: none; }
.flat:hover { background: rgba(52, 152, 219, 0.25); }
.flat:checked { background: rgba(52, 152, 219, 0.3); }
colorbutton.flat, colorbutton.flat button { background: none; }
colorbutton.flat:hover, colorbutton.flat button:hover { background: rgba(52, 152, 219, 0.25); }
dropdown.flat, dropdown.flat button { background: none; border-radius: 5px; }
dropdown.flat:hover { background: rgba(52, 152, 219, 0.25); }
.flat-header { background: rgba(52, 152, 219, 0.08); border: none; box-shadow: none; padding: 0px; }
.button-box button { min-width: 80px; min-height: 36px; }
.highlighted { background-color: rgba(52, 152, 219, 0.18); }
.toolbar-group { margin: 0px 3px; }
.toolbar-separator { min-height: 0px; min-width: 1px; background-color: alpha(currentColor, 0.15); margin: 0px 0px; }
.color-indicator { min-height: 2px; min-width: 16px; margin-top: 1px; margin-bottom: 0px; border-radius: 2px; }
.color-box { padding: 0px; }

.linked button               {background: @theme_bg_color; border: 1px solid rgba(52, 152, 219, 0.2); box-shadow: 0 4px 20px rgba(52, 152, 219, 0.15);}
.linked button:hover         {background-color: rgba(52, 152, 219, 0.35); border: solid 1px rgba(52, 152, 219, 0.4);}
.linked button:active        {background-color: rgba(52, 152, 219, 0.5); border: solid 1px rgba(52, 152, 219, 0.6);}
.linked button:checked       {background-color: rgba(52, 152, 219, 0.4); border: solid 1px rgba(52, 152, 219, 0.5);}
.linked button:checked:hover {background-color: rgba(52, 152, 219, 0.6); border: solid 1px rgba(52, 152, 219, 0.7);}
""" + self.get_common_css()
        }
    ...
    def get_green_theme(self):
        ...
        return {
            "name": "Forest Green",
            "css": """
.toolbar-container { padding: 0px 0px; background-color: rgba(46, 204, 113, 0.08); }
.flat { background: none; }
.flat:hover { background: rgba(46, 204, 113, 0.25); }
.flat:checked { background: rgba(46, 204, 113, 0.3); }
colorbutton.flat, colorbutton.flat button { background: none; }
colorbutton.flat:hover, colorbutton.flat button:hover { background: rgba(46, 204, 113, 0.25); }
dropdown.flat, dropdown.flat button { background: none; border-radius: 5px; }
dropdown.flat:hover { background: rgba(46, 204, 113, 0.25); }
.flat-header { background: rgba(46, 204, 113, 0.08); border: none; box-shadow: none; padding: 0px; }
.button-box button { min-width: 80px; min-height: 36px; }
.highlighted { background-color: rgba(46, 204, 113, 0.18); }
.toolbar-group { margin: 0px 3px; }
.toolbar-separator { min-height: 0px; min-width: 1px; background-color: alpha(currentColor, 0.15); margin: 0px 0px; }
.color-indicator { min-height: 2px; min-width: 16px; margin-top: 1px; margin-bottom: 0px; border-radius: 2px; }
.color-box { padding: 0px; }

.linked button               {background: @theme_bg_color; border: 1px solid rgba(46, 204, 113, 0.2); box-shadow: 0 4px 20px rgba(46, 204, 113, 0.15);}
.linked button:hover         {background-color: rgba(46, 204, 113, 0.35); border: solid 1px rgba(46, 204, 113, 0.4);}
.linked button:active        {background-color: rgba(46, 204, 113, 0.5); border: solid 1px rgba(46, 204, 113, 0.6);}
.linked button:checked       {background-color: rgba(46, 204, 113, 0.4); border: solid 1px rgba(46, 204, 113, 0.5);}
.linked button:checked:hover {background-color: rgba(46, 204, 113, 0.6); border: solid 1px rgba(46, 204, 113, 0.7);}
""" + self.get_common_css()
        }
    
    def get_purple_theme(self):
        """Purple creative theme"""
        return {
            "name": "Royal Purple",
            "css": """
.toolbar-container { padding: 0px 0px; background-color: rgba(155, 89, 182, 0.08); }
.flat { background: none; }
.flat:hover { background: rgba(155, 89, 182, 0.25); }
.flat:checked { background: rgba(155, 89, 182, 0.3); }
colorbutton.flat, colorbutton.flat button { background: none; }
colorbutton.flat:hover, colorbutton.flat button:hover { background: rgba(155, 89, 182, 0.25); }
dropdown.flat, dropdown.flat button { background: none; border-radius: 5px; }
dropdown.flat:hover { background: rgba(155, 89, 182, 0.25); }
.flat-header { background: rgba(155, 89, 182, 0.08); border: none; box-shadow: none; padding: 0px; }
.button-box button { min-width: 80px; min-height: 36px; }
.highlighted { background-color: rgba(155, 89, 182, 0.18); }
.toolbar-group { margin: 0px 3px; }
.toolbar-separator { min-height: 0px; min-width: 1px; background-color: alpha(currentColor, 0.15); margin: 0px 0px; }
.color-indicator { min-height: 2px; min-width: 16px; margin-top: 1px; margin-bottom: 0px; border-radius: 2px; }
.color-box { padding: 0px; }

.linked button               {background: @theme_bg_color; border: 1px solid rgba(155, 89, 182, 0.2); box-shadow: 0 4px 20px rgba(155, 89, 182, 0.15);}
.linked button:hover         {background-color: rgba(155, 89, 182, 0.35); border: solid 1px rgba(155, 89, 182, 0.4);}
.linked button:active        {background-color: rgba(155, 89, 182, 0.5); border: solid 1px rgba(155, 89, 182, 0.6);}
.linked button:checked       {background-color: rgba(155, 89, 182, 0.4); border: solid 1px rgba(155, 89, 182, 0.5);}
.linked button:checked:hover {background-color: rgba(155, 89, 182, 0.6); border: solid 1px rgba(155, 89, 182, 0.7);}
""" + self.get_common_css()
        }
    ...
    def get_dark_theme(self):
        ...
        return {
            "name": "Dark Orange",
            "css": """
.toolbar-container { padding: 0px 0px; background-color: rgba(230, 126, 34, 0.12); }
.flat { background: none; }
.flat:hover { background: rgba(230, 126, 34, 0.3); }
.flat:checked { background: rgba(230, 126, 34, 0.4); }
colorbutton.flat, colorbutton.flat button { background: none; }
colorbutton.flat:hover, colorbutton.flat button:hover { background: rgba(230, 126, 34, 0.3); }
dropdown.flat, dropdown.flat button { background: none; border-radius: 5px; }
dropdown.flat:hover { background: rgba(230, 126, 34, 0.3); }
.flat-header { background: rgba(230, 126, 34, 0.12); border: none; box-shadow: none; padding: 0px; }
.button-box button { min-width: 80px; min-height: 36px; }
.highlighted { background-color: rgba(230, 126, 34, 0.25); }
.toolbar-group { margin: 0px 3px; }
.toolbar-separator { min-height: 0px; min-width: 1px; background-color: alpha(currentColor, 0.15); margin: 0px 0px; }
.color-indicator { min-height: 2px; min-width: 16px; margin-top: 1px; margin-bottom: 0px; border-radius: 2px; }
.color-box { padding: 0px; }

.linked button               {background: @theme_bg_color; border: 1px solid rgba(230, 126, 34, 0.3); box-shadow: 0 4px 20px rgba(230, 126, 34, 0.2);}
.linked button:hover         {background-color: rgba(230, 126, 34, 0.4); border: solid 1px rgba(230, 126, 34, 0.5);}
.linked button:active        {background-color: rgba(230, 126, 34, 0.6); border: solid 1px rgba(230, 126, 34, 0.7);}
.linked button:checked       {background-color: rgba(230, 126, 34, 0.5); border: solid 1px rgba(230, 126, 34, 0.6);}
.linked button:checked:hover {background-color: rgba(230, 126, 34, 0.7); border: solid 1px rgba(230, 126, 34, 0.8);}
""" + self.get_common_css()
        }
    
    def get_minimal_theme(self):
        """Minimal theme with subtle effects"""
        return {
            "name": "Minimal",
            "css": """
.toolbar-container { padding: 0px 0px; background-color: rgba(0, 0, 0, 0.02); }
.flat { background: none; }
.flat:hover { background: rgba(0, 0, 0, 0.08); }
.flat:checked { background: rgba(0, 0, 0, 0.12); }
colorbutton.flat, colorbutton.flat button { background: none; }
colorbutton.flat:hover, colorbutton.flat button:hover { background: rgba(0, 0, 0, 0.08); }
dropdown.flat, dropdown.flat button { background: none; border-radius: 5px; }
dropdown.flat:hover { background: rgba(0, 0, 0, 0.08); }
.flat-header { background: rgba(0, 0, 0, 0.02); border: none; box-shadow: none; padding: 0px; }
.button-box button { min-width: 80px; min-height: 36px; }
.highlighted { background-color: rgba(0, 0, 0, 0.06); }
.toolbar-group { margin: 0px 3px; }
.toolbar-separator { min-height: 0px; min-width: 1px; background-color: alpha(currentColor, 0.1); margin: 0px 0px; }
.color-indicator { min-height: 2px; min-width: 16px; margin-top: 1px; margin-bottom: 0px; border-radius: 2px; }
.color-box { padding: 0px; }

.linked button               {background: @theme_bg_color; border: 1px solid rgba(0, 0, 0, 0.05); box-shadow: 0 2px 8px rgba(0, 0, 0, 0.08);}
.linked button:hover         {background-color: rgba(0, 0, 0, 0.08); border: solid 1px rgba(0, 0, 0, 0.12);}
.linked button:active        {background-color: rgba(0, 0, 0, 0.15); border: solid 1px rgba(0, 0, 0, 0.2);}
.linked button:checked       {background-color: rgba(0, 0, 0, 0.12); border: solid 1px rgba(0, 0, 0, 0.15);}
.linked button:checked:hover {background-color: rgba(0, 0, 0, 0.18); border: solid 1px rgba(0, 0, 0, 0.25);}
""" + self.get_common_css()
        }
    
    def get_common_css(self):
        """Common CSS that's shared across all themes"""
        return """
.linked dropdown listview {
        margin: 0px;
    }
...
"""
    ...
    def get_theme_names(self):
        ...
        return [self.themes[key]["name"] for key in self.themes.keys()]
    
    def get_theme_keys(self):
        """Get list of theme keys"""
        return list(self.themes.keys())
    
    def get_theme_css(self, theme_key):
        """Get CSS for a specific theme"""
        if theme_key in self.themes:
            return self.themes[theme_key]["css"]
        return self.themes["default"]["css"]
    
    def set_current_theme(self, theme_key):
